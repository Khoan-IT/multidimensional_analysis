...
for (col in colnames(data)){
  cap[col] <- paste("Boxplot of Diagnosis vs.", gsub('_', ' ', colnames(data[col])))
  #cap[col] <- paste("$Boxplot of Diagnosis vs.", gsub('_', '\\_', colnames(data[col])), '$')
}
```
...
for (col in colnames(data)){
  if (col != 'diagnosis'){
    boxplot(unlist(data[, col], use.names=FALSE) ~ data$diagnosis, xlab='Diagnosis', ylab=col, main = NULL)
  }
}
```
* Histogram 
...
for (col in colnames(data)){
  cap[col] <- paste("Histogram of", gsub('_', ' ', colnames(data[col])))
  #cap[col] <- paste("$\\text{Histogram of", gsub('_', '_', colnames(data[col])), '}$')
  #cap[col] <- paste("$\\text{Histogram of", colnames(data[col]), '}$')
}
```
...
for (col in colnames(data)){
  if(col != 'diagnosis'){
    hist(unlist(data[, col], use.names=FALSE), xlab=col, main = NULL)
  }
  else{
...
```{r, tidy=FALSE, fig.align='center', fig.width=100, fig.height=100, fig.cap="Scatter plot of wine dataset"}
  pairs(
...
```{r,  tidy=FALSE, fig.align='center', fig.width=5, fig.height=5, fig.cap="Correlation plot of wine dataset"}
data.cor <- cor(data[-1], method = "pearson")
corrplot(
  data.cor, 
  is.corr=F, 
  type = "lower", 
