...
| 2014482 	| Tran Trung Thai       	| Computer Science  |
| 1912046 	| Ngo Minh Hong Thai     	| Computer Science  |
| 2011291 	| Tran Hoang Nhat Huy    	| Computer Science  |
...
```{r, tidy=FALSE, fig.align='center', out.width="55%", fig.cap=cap[2:31]}
for (col in colnames(data)){
  if (col != 'diagnosis'){
    boxplot(
      data[, col] ~ data[, 'diagnosis'], 
      xlab='diagnosis', 
      ylab=col, 
      main = NULL
    ) 
...
```{r, tidy=FALSE, fig.align='center', out.width="55%", fig.cap=cap}
for (col in colnames(data)){
  if(col != 'diagnosis'){
...
```{r, tidy=FALSE, fig.align='center', fig.width=4, fig.height=4, fig.cap=c("Hierarchical Clustering plot", "Bar plot of Type vs. cluster percent")}
...
```{r, fig.align='center', fig.width=4, fig.height=4, fig.cap=c("Hierarchical Clustering with Single Linkage")}
hc <- agnes(scaled_data, method = "single")
sub_grp <- cutree(hc, k = 2)
fviz_cluster(list(data = scaled_data, cluster = sub_grp))
```
```{r, fig.align='center', fig.width=4, fig.height=4, fig.cap=c("Hierarchical Clustering with Ward Linkage")}
...
```{r}
    # Correlation tests or t-tests comparing mean of smoothness \
    # and perimeter across benign and malignant tumors.
    label <- factor(data$diagnosis)
...
plot(
  smoothness_mean ~ as.factor(diagnosis), 
  data = data, 
  xlab = "Tumors", ylab = "Mean of smoothness", 
  main = "Mean of smoothness across Tumors"
)
plot(
  perimeter_mean ~ as.factor(diagnosis), 
  data = data, 
  xlab = "Tumors", ylab = "Mean of perimeter", 
  main = "Mean of perimeter cross Tumors"
)
...
In this report, we explored and analyzed a data set in the medical field - the Breast Cancer Wisconsin dataset. The analysis process, particularly from PCA, delineate the pivotal role of specific features related to breast mass characteristics derived from digitized images of FNA of breast masses in diagnosing the existing of breast cancer. As we've analyzed in the previous section, it can be seen that there are many factors that affect the results of diagnosis through FNA (Fine Needle Aspirate). Specifically, the factors related to the Concavity of cell nuclei, as all the metrics of this factor are discriminative for the diagnostic results according to the PCA analysis and ANOVA testing results. This implies that Concavity is a defining characteristic in classifying tumor types. Also, the mean and the worst metrics on other factors like Nucleus Radius, Perimeter, Area and Compactness also affect the diagnosis result. Therefore, it is necessary to focus on these two measurements during the diagnostic process based on FNA.

With hierarchical clustering analysis, the study further emphasized the heterogeneity in the benign and malignant breast cancer patient groups, as evident by the clear division into two clusters (M or B tumors). Also. the conducted ANOVA tests further validate and emphasize the significance of these components in distinguishing between the tumor types.
...
For the purpose of assisting in the process of diagnosing breast cancer tumors, one of the most dangerous and relatively common diseases today, patients and also experts should focus on the factor of Concavity of NFA results. The larger the values of Worst and Mean Concavity, the more likely it is that the diagnosis tumors are malignant. In addition, the factors such as Radius, Perimeter, Area and Compactness also need to be focused as they all have a relatively significant influence on the diagnostic results. For all the measurements mentioned above, metrics Mean and Worst should be more focused than Standard Error as these are the metrics that best demonstrate the distinction between tumor types. The elevating values of these metrics are associated with a greater likelihood of tumors being diagnosed as malignant.

Thereby, the analysis and evaluation of the above factors bring many meanings in diagnosing patients' breast cancer. Focusing on important factors can help make the diagnosis more accurate and faster. Thereby, patients are diagnosed and treated promptly, avoiding future risks.

## **Limitations**

Although factors affecting diagnosis results have been identified through analyzing data on breast cancer tumors. However, the data being used is still not large enough to provide the most general assessment of NFA results. Therefore, collecting more analytical data is necessary for the generalization of analyzing results and making suggestions on which factors to focus on. Also, with bigger and more general dataset, we can provide the most accurate analysis results, avoiding misunderstanding and confusion for patients.

In addition, Hierarchical clustering (HC) models have achieved significant results, but are still not capable of distinguishing between diagnostic results. In the future, more powerful models in the classification problem can be used such as Support Vector Machine (SVM) or Neural Network can be applied. From there, a suggestion model with higher accuracy was built, supporting the diagnosis of breast cancer for patients.
